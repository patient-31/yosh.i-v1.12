...
## Changes
 ...
 - outputed error message when using more than one pipe.

 # 1.12 (being developed on macOS 15)
 - finally fixed bug where incomplete env variables would be printed from echo (retrv_envv()).
 - removed libft and moved sections of library to utils.c
 - altered makefile to target brew version of readline 8.2.13 on macOS 15.
 - completely altered makefiles compilation step so that an object folder is created.
